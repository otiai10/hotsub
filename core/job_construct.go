...
func (job *Job) wakeupRoutineContainer() error {
	...
	err = container.Create(ctx, daap.CreateConfig{
		Host: &dockercontainer.HostConfig{
			Mounts:     []mount.Mount{daap.Bind(HotsubHostRoot, HotsubContainerRoot)},
			Privileged: true,
		},
	})
	if err != nil {
...
func (job *Job) wakeupWorkflowContainer(volumes []*daap.Volume) error {
	...
	mounts := []mount.Mount{
		daap.Bind(HotsubHostRoot, HotsubContainerRoot),
	}
	for _, volume := range volumes {
		...
		mounts = append(
			mounts,
			daap.VolumeByName(volume.Name, HotsubContainerRoot+"/"+HotsubSharedDirectoryPath),
		)
	}
