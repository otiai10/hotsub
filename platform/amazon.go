...
func (p AmazonWebServices) Validate(ctx params.Context) error {
	...
	if err := p.createWorkspaceVpcIfNotExists(sess, ctx); err != nil {
		return fmt.Errorf("failed to setup VPC: %v", err)
	}

	if err := p.createSecurityGroupIfNotExists(sess, ctx); err != nil {
		return fmt.Errorf("failed to setup security group: %v", err)
	}

	if err := p.createInstanceProfileIfNotExists(sess, ctx); err != nil {
